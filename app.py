...
app = Flask(__name__)
app.secret_key = 'gemyeong1013@gmail.com'
...
try:
    ...
    for y in range(2,8):
        for x in range(2,7):
            conn = sqlite3.connect(databass)
            c = conn.cursor()
            content = driver.find_element_by_xpath(f'/html/body/div/div/section/div/div/div[2]/div[2]/div/table/tbody/tr[{y}]/td[{x}]/div/div/div/div[2]/span/a').text
            c.execute('INSERT INTO lunch (content) VALUES (?)', (content,))
            conn.commit()
        conn.close()
...
def webhook_send(sendContent):
    webhook = DiscordWebhook(url='https://discord.com/api/webhooks/952714072963563620/f7RJpddGsIwFLDxgVbbjPEmTP7MtUMWIOvOEDb83negCjtMkoBhh4gQgovZWsNDq6eip', content=sendContent)
...
def index():
    ...
    conn.close()
    reRow = [s[0].replace('\n', '\n') for s in row]
    return render_template('index.html', rows=reRow)
